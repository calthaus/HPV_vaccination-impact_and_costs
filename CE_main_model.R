...
HPV_1<- function(t, x, params, verbose=FALSE)
{
  with(as.list(params), {
    ...
    p2v_p <- if(w==1 | w==2)  p2v else pv0
    p4v_p <- if(w==1 | w==2 | w== 11 | w==12)  p4v else  pv0
    p9v_p <- if(w==1 | w==2 |w==3 |w==4 | w==7 |w==9 |w==10 |  w== 11 | w==12)  p9v else  pv0
    lambda <- array(NA, dim=c(length(sex),length(riskgroups),length(agegroups)) )
    beta <- c(beta_fm, beta_mf)
    for (i in 1:length(sex)){
      for (j in 1:length(riskgroups)){
        for(a in 1:length(agegroups)){
          
          lambda[i,j,a] <- beta[i]  * sum( rho[i,j,1:length(riskgroups),a,1:length(agegroups) ]* Cp[i,j,1:length(riskgroups),a,1:length(agegroups) ]*
                                             ( X[2,ifelse(i==1, 2, 1), 1:length(riskgroups), 1:length(agegroups)]/
                                                 N[ifelse(i==1, 2, 1), 1:length(riskgroups), 1:length(agegroups)] ) )
          
          ###1 Susceptible    
          dX[1,i,j,a] <- (  
            ifelse(a>1, g[a-1]*X[1,i,j,a-1],  (1- (p2v_p[i,j,a] + p4v_p[i,j,a]+ p9v_p[i,j,a] )*ve[w] ) * g[length(agegroups)]*N[i,j,length(agegroups) ] )
            - lambda[i,j,a]*X[1,i,j,a] 
            + omega[i,w]*X[12,i,j,a]
            + (1-alpha[w]) * gamma[i,w]* X[2,i,j,a] # percentage that does not seroconvert
            - g[a]*X[1,i,j,a]
            + s[i,a,y]*ssens* X[3,i,j,a] # screened women CIN2
            + s[i,a,y]*ssens* X[4,i,j,a] # screened women CIN3
            + pr2s[w,y]*X[3,i,j,a] #regression from cin2 to normal
            + pr3s[w,y]*X[4,i,j,a] #regression from cin3 to normal
            - m*X[1,i,j,a] + m*rgr[j]*(X[1,i,j,a]+X[1,i,ifelse(j==1,2,1),a])
          ...
          dX[2,i,j,a] <-  (lambda[i,j,a]* (X[1,i,j,a] + tau[w,y]*X[12,i,j,a]) 
                           ...
                           - gamma[i,w]*X[2,i,j,a]
                           + ifelse(a>1, g[a-1]*X[2,i,j,a-1],0)
                           - g[a]*X[2,i,j,a]   
                           - m*X[2,i,j,a] + m*rgr[j]*(X[2,i,j,a]+X[2,i,ifelse(j==1,2,1),a]))
          ...
          dX[3,i,j,a] <-  (pri1[i,w,y]*X[2,i,j,a] 
                           ...
                           - pr2c[w]*X[3,i,j,a] 
                           - pr2s[w,y]*X[3,i,j,a]
                           + ifelse(a>1, g[a-1]*X[3,i,j,a-1],0) # aging in
                           - g[a]*X[3,i,j,a]   #aging out
                           +  pr32*X[4,i,j,a] 
                           - s[i,a,y]*ssens*X[3,i,j,a] # screened women
                           - m*X[3,i,j,a] + m*rgr[j]*(X[3,i,j,a]+X[3,i,ifelse(j==1,2,1),a]))
          ...
          dX[4,i,j,a] <-  (pri2[i,w,y]*X[2,i,j,a]
                           ...
                           - pr32*X[4,i,j,a]
                           - pr3s[w,y]*X[4,i,j,a]
                           + ifelse(a>1, g[a-1]*X[4,i,j,a-1],0)
                           - g[a]*X[4,i,j,a] 
                           - s[i,a,y]*ssens*X[4,i,j,a] 
                           - m*X[4,i,j,a] + m*rgr[j]*(X[4,i,j,a]+X[4,i,ifelse(j==1,2,1),a])) 
          ...
          dX[5,i,j,a] <-  (pr2c[w]*X[3,i,j,a] #progression CIN2 to ULCC
                           ...
                           + ifelse(a>1, g[a-1]*X[5,i,j,a-1],0) #aging in
                           ...
                           - m*X[5,i,j,a] + m*rgr[j]*(X[5,i,j,a]+X[5,i,ifelse(j==1,2,1),a])) 
          ...
          dX[6,i,j,a] <-  ( (1-pi[1, y] )* z[1,y]*X[5,i,j,a] #in: stage spec. prob of diagn. (times 1- probability of cure with treatm.)
                            + ifelse(a>1, g[a-1]*X[6,i,j,a-1],0) #aging in
                            - g[a]*X[6,i,j,a]  #aging out
                            - dm[1] * X[6,i,j,a] #diagnosed mortality
                            - m*X[6,i,j,a] + m*rgr[j]*(X[6,i,j,a]+X[6,i,ifelse(j==1,2,1),a])) 
          
          #7 undiagnosed regional cervical cancer
          dX[7,i,j,a] <-  (prlcrc*X[5,i,j,a] 
                           + ifelse(a>1, g[a-1]*X[7,i,j,a-1],0) #aging in
                           ...
                           - m*X[7,i,j,a] + m*rgr[j]*(X[7,i,j,a]+X[7,i,ifelse(j==1,2,1),a])) 
          ...
          dX[10,i,j,a] <-  ( (1-pi[3,y] )* z[3,y]*X[9,i,j,a] #in: stage spec. prob of diagn. (times 1- probability of cure with treatm.)
                             + ifelse(a>1, g[a-1]*X[10,i,j,a-1],0) #aging in
                             - g[a]*X[10,i,j,a]  #aging out
                             - dm[3] * X[10,i,j,a] #diagnosed mortality
                             - m*X[10,i,j,a] + m*rgr[j]*(X[10,i,j,a]+X[10,i,ifelse(j==1,2,1),a])) 
          
          #11 deaths from CC
          dX[11,i,j,a] <-   ( sum(dm[1:3]* X[c(6,8,10),i,j,a])
                              +sum(um[1:3]* X[c(5,7,9),i,j,a])
                              + ifelse(a>1, g[a-1]*X[11,i,j,a-1],0) #aging in
          ...
          dX[12,i,j,a] <-  (  alpha[w] * gamma[i,w]* X[2,i,j,a]
                              ...
                              + ifelse(a>1, g[a-1]*X[12,i,j,a-1],0)
                              - g[a]*X[12,i,j,a]  
                              - m*X[12,i,j,a] + m*rgr[j]*(X[12,i,j,a]+X[12,i,ifelse(j==1,2,1),a]) ) 
          #13 V2v
          dX[13,i,j,a] <-  (ifelse(a>1, g[a-1]*X[13,i,j,a-1], p2v_p[i,j,a]*g[length(agegroups)]*ve[w]*N[i,j,length(agegroups) ] )
                            - g[a]*X[13,i,j,a] 
                            - m*X[13,i,j,a] + m*rgr[j]*(X[13,i,j,a]+X[13,i,ifelse(j==1,2,1),a])) 
          
          #14 V4v
          dX[14,i,j,a] <-  (ifelse(a>1, g[a-1]*X[14,i,j,a-1], p4v_p[i,j,a]*g[length(agegroups)]*ve[w] *N[i,j,length(agegroups) ] )
                            - g[a]*X[14,i,j,a] 
                            - m*X[14,i,j,a] + m*rgr[j]*(X[14,i,j,a]+X[14,i,ifelse(j==1,2,1),a])) 
          
          #15 V9v
          dX[15,i,j,a] <-  (ifelse(a>1, g[a-1]*X[15,i,j,a-1], p9v_p[i,j,a]*g[length(agegroups)]*ve[w] *N[i,j,length(agegroups) ] )
                            - g[a]*X[15,i,j,a] 
                            - m*X[15,i,j,a] + m*rgr[j]*(X[15,i,j,a]+X[15,i,ifelse(j==1,2,1),a])) 
          # setTxtProgressBar(pba, a)
          
          # if(verbose){
          #   cat("time: ", paste(t, collapse = ";"), '\n')
          #   cat("w", paste(w, collapse = ";"), '\n')
          #   cat("vacc cost: ", paste(sum( X[19,1,1,1 ]  ) , collapse = ";"), '\n') }
