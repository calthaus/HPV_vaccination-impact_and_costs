...
# plot standardised cervical cancer incidence, over years
# pdf("cc_standardised_CH_v2.pdf", width=8, height=6)
plot(dat_cc$Période, dat_cc$`Taux standardisé`, ylim=c(0,12), xlim=c(1990,2014) , xlab="year", ylab="incidence rate / 100'000 person-year", 
     main="Standardised cervical cancer incidence in Switzerland", frame=F, pch=16, cex=1.5, xaxt="n")
axis(1, at= seq(1990,2014, by=2) )
...
totinc_cc <- mean( dat_cc$`Taux standardisé`[dat_cc$Période>2003])

totcancer <- (totinc_cc/ 1e5)* (popmod*1e6 *sr ) 
...
#              "hpv51", "hpv52", "hpv56" ,"hpv58", "hpv59", "hpv66", "other_HR","hpv6", "hpv11" )
hpvtype <- c("hpv16", "hpv18", "hpv31", "hpv33", "hpv35", "hpv39", "hpv45",
...
#################### Other cancer stages incidences linked with cervical cacner, type ratio from different sources
...
################# % covered in nonavalent vaccines
if(w==1 | w==2 |w==3 |w==4 | w==7 |w==9 |w==10 |  w== 11 | w==12) 

sum( cc_inc_pertype2[c(1,2,3,4,7,9,10,11,12)] ) / sum(cc_inc_pertype2)
